...
namespace
{
    ...
    {
        if (tag.find("Submissions") != std::string::npos)
            return Section::SUBMISSIONS;
        if (tag.find("Reviewers") != std::string::npos)
            return Section::REVIEWERS;
        if (tag.find("Parameters") != std::string::npos)
            return Section::PARAMETERS;
        if (tag.find("Control") != std::string::npos)
        ...
        else if (current == Section::PARAMETERS || current == Section::CONTROL)
        {
            ...
            try
            {
                if (key == "MinReviewsPerSubmission")
                    params.MinReviewsPerSubmission = std::stoi(val);
                else if (key == "MaxReviewsPerReviewer")
                    params.MaxReviewsPerReviewer = std::stoi(val);
                else if (key == "PrimaryReviewerExpertise")
                    params.PrimaryReviewerExpertise = std::stoi(val);
                else if (key == "SecondaryReviewerExpertise")
                    params.SecondaryReviewerExpertise = std::stoi(val);
                else if (key == "PrimarySubmissionDomain")
                    params.PrimarySubmissionDomain = std::stoi(val);
                else if (key == "SecondarySubmissionDomain")
                    params.SecondarySubmissionDomain = std::stoi(val);
                else if (key == "GenerateAssignments")
                    params.GenerateAssignments = std::stoi(val);
                else if (key == "RiskAnalysis")
                    params.RiskAnalysis = std::stoi(val);
                else if (key == "OutputFileName")
                    params.OutputFileName = val;
            }
            catch (const std::exception &e)
            {
                errors.push_back("Line " + std::to_string(lineNum) +
