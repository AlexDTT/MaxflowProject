...
int GraphBuilder::sourceId() {
    return 0;
}

// Submissions occupy nodes 1..P
int GraphBuilder::submissionNodeId(int subIndex) {
    return 1 + subIndex;
}

// Reviewers occupy nodes P+1..P+R
int GraphBuilder::reviewerNodeId(int revIndex, int numSubs) {
    return 1 + numSubs + revIndex;
}

// Sink is node P+R+1
int GraphBuilder::sinkId(int numSubs, int numRevs) {
    return 1 + numSubs + numRevs;
}

int GraphBuilder::getMatchedDomain(const Submission& sub, const Reviewer& rev, int mode) {
...
Graph<int> GraphBuilder::buildReviewFlowGraph(
...
string GraphBuilder::getNodeLabel(int id, const vector<Submission>& submissions, const vector<Reviewer>& reviewers) {
    int P = (int)submissions.size();
    int R = (int)reviewers.size();
    int src = sourceId();
    int snk = sinkId(P, R);
...
void GraphBuilder::printFlowGraph(
        const Graph<int>&         g,
        const vector<Submission>& submissions,
        const vector<Reviewer>&   reviewers) {
